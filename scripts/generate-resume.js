...
/**
 * @brief Maps full month names to r/EngineeringResumes-standard abbreviations
 * @details Jan/Feb/Mar/Apr/Aug/Sept/Oct/Nov/Dec are abbreviated; May/June/July are already short
 */
...
/**
 * @brief Abbreviates full month names per resume formatting conventions
 * @param text The date/duration string to normalize
 * @return Date string with full month names replaced by their standard abbreviation
 */
...
/**
 * @brief Converts hyphenated date ranges to en dash ranges for LaTeX output
 * @param text The date/duration string to format
 * @return Date string with space-padded hyphens converted to en dashes and months abbreviated
 * @details Only matches a hyphen surrounded by whitespace, leaving compound words untouched
 */
function formatDateRange(text) {
    if (typeof text !== 'string') return text;
    return abbreviateMonths(text).replace(/\s-\s/g, ' – ');
}
...
const filters = {
    filterResumeExperience(experiences) {
        return experiences.filter((exp) => exp.Visibility === 'All');
    },
    filterResumeProjects(projects) {
        return projects.filter((project) => project.Visibility === 'All');
    },
    ...
    filterResumeCourses(courses) {
        ...
        Object.values(courses).forEach((categoryCourses) => {
            const filteredCourses = categoryCourses.filter((course) => course.Visibility === 'All');
            allCourses.push(...filteredCourses);
        });

        return allCourses.sort((left, right) => {
    ...
    shouldShowResumeProfile(profile) {
        ...
        return profile.Visibility === 'All';
    },
    getProfileText(profile, mode = 'Highlights') {
        if (!profile) return null;
        if (typeof profile === 'string') return profile;

        const resumeText = profile.Resume || profile.text || profile.Highlights || null;
        const highlightsText = profile.Highlights || profile.Resume || profile.text || null;
    ...
    filterWebsiteSkills(skills) {
        ...
        Object.entries(skills).forEach(([category, skillList]) => {
            filtered[category] = skillList.filter(
                (skill) => skill.Visibility === 'All' || skill.Visibility === 'Site'
            );
        });
        return filtered;
    },
    filterWebsiteCertifications(certifications) {
        return certifications.filter(
            (cert) => cert.Visibility === 'All' || cert.Visibility === 'Site' || !cert.Visibility
        );
    },
};
...
function calculateSpacingProfile({
    ...
    function estimateTotalLines(sp) {
        ...
        resumeProjects.forEach((project, idx) => {
            ...
            const items = Array.isArray(project.Resume) ? project.Resume : [];
            const bulletLines = items.reduce(
                (sum, item) => sum + estimateWrappedLines(item, CHARS_PER_LINE),
                0
            );
...
function generateResume(data, options = {}) {
    ...
    if (includeMS) {
        const msEducations = data.education.filter((edu) => {
            ...
            return (
                (degree.startsWith('ms') || degree.includes('master')) &&
                !resumeEducations.includes(edu)
            );
        });
    ...
    const courseNames = resumeCourses
        .map((course) => {
            if (
                (course.alias === 'Signals and Systems I' ||
                    course.alias === 'Signals and Systems II') &&
                hasSS1 &&
                hasSS2
            ) {
                return null;
    ...
    const otherEducationBlocks = shouldStackMsBs
        ...
        : otherEducations
              .map((ed) => {
                  ...
                  const expected = ed.graduationDate ? `${escapeLatex(ed.graduationDate)}` : '';
                  lines.push(
    ...
    const buildEducationBody = () => {
        if (shouldStackMsBs && mastersEntry) {
            ...
            const msExpected = mastersEntry.graduationDate
                ? ` \\hfill ${escapeLatex(mastersEntry.graduationDate)}`
                : '';
            lines.push(
                `\\textbf{${escapeLatex(mastersEntry.degree)}}${mastersEntry.specialization ? ` -- ${escapeLatex(mastersEntry.specialization)}` : ''}${msExpected} \\\\`
            );
            if (bachelorsEntry) {
                const bsExpected = bachelorsEntry.graduationDate
                    ? ` \\hfill ${escapeLatex(bachelorsEntry.graduationDate)}`
                    : '';
                const bsDegreeNorm = String(bachelorsEntry.degree || '')
                    .replace(/\b[in]\b\s*/i, ' ')
                    .replace(/\s{2,}/g, ' ')
                    .trim();
                const bsMinorPart = bachelorsEntry.minor
                    ? `, ${escapeLatex(bachelorsEntry.minor)}`
                    : '';
                lines.push(
                    `\\textbf{${escapeLatex(bsDegreeNorm)}}${bsMinorPart}${bsExpected} \\\\`
                );
            }
            return lines.join('\n');
        }
        ...
        if (eduToDisplay) {
            const lines = [];
            lines.push(
                `\\textbf{${escapeLatex(eduToDisplay.institution)}} ${eduToDisplay.graduationDate ? `\\hfill ${escapeLatex(eduToDisplay.graduationDate)}` : ''} \\\\`
            );
    ...
    if (data.personal.email)
        contactParts.push(
            `\\href{mailto:${data.personal.email}}{${escapeLatex(data.personal.email)}}`
        );
