...
// Rough estimate of how many lines text will wrap to (for spacing calculations)
...
// Auto-fit content to target page count by adjusting vertical spacing.
// Keeps things ATS-friendly (no columns or fancy layouts).
function calculateSpacingProfile({ data, includeProfile, includeMS, shouldStackMsBs, targetPages = 1 }) {
	...
	function estimateTotalLines(sp) {
		...
		resumeExperience.forEach((exp, idx) => {
			...
			const bulletLines = items.reduce((sum, item) => sum + estimateWrappedLines(item, CHARS_PER_LINE), 0);
	...
	if (process.env.RESUME_DEBUG_SPACING === '1') {
		...
		console.log('[resume spacing] estimatedLines:', spFinal._debug.estimatedLines.toFixed(1), '/', spFinal._debug.capLines.toFixed(1));
...
function generateResume(data, options = {}) {
	...
	const { primaryEducation } = filters.getEducationContext(data.education, data.educationSupplementary) || {};
	...
	const experienceSection = resumeExperience.map(exp =>
...
\\end{itemize}`
	...
	const projectsSection = resumeProjects.map(project =>
...
\\end{itemize}`
	...
	const profileSection = profileText ? `\\section*{Profile}%
...
// Two versions: with and without profile summary
const variations = [
	{ name: 'Edward_Silva_Resume.tex', options: { includeMS: false, includeProfile: true, targetPages: 1 } }
];

// Remove old variants that shouldn't exist anymore
try {
	const allowed = new Set(variations.map(v => v.name));
