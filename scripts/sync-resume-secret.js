...
const resumePath = path.resolve(root, 'resume-data.json');
...
function validateResumeFile() {
    ...
    try {
        ...
        console.error(`Local resume data file is missing or invalid JSON: ${resumePath}`);
...
function syncSecretToGithub(jsonText, secretName, scope, repository, environment) {
    ...
    console.log(`Updated ${scopeLabel} secret ${secretName} in ${repository} from ${resumePath}`);
}
